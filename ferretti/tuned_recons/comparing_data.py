...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 23 16:21:27 2025
...
class ProjPBI(nn.Module): 
    ...
    def __call__(self, angle: float) -> Array:
        ...
        volume = np.pad(volume,((total_pad,total_pad),(total_pad,total_pad),(0,0),(0,0)),'constant', constant_values=0)
...
class MultiSlicePBI(nn.Module): 
    ...
    def __call__(self, angle: float) -> Array:
        ...
        # to detector
        det_field = cx.transfer_propagate(exit_field, self.propdist, self.n_medium, N_pad, cval=model.cval, mode='same')
        img = self.det_resample_func(det_field.intensity.squeeze()[...,None,None], field.dx.ravel()[:1])[...,0,0]
        img = img / (self.det_dx/(self.phantom_dx/up_samp_fac))**2  # normalize counts to new pixel size
        img = img.swapaxes(0,1)      
...
data_1 = forward_1(params, thetas)
...
# Simulate
forward_2 = jax.vmap(model.apply, in_axes=(None, 0))
data_2 = forward_2(params, thetas)
