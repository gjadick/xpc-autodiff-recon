...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 24 12:51:25 2025
...
class ProjPBI(nn.Module): 
    ...
    # Phantom
    phantom_Nx: int = 16#64
    phantom_Ny: int = 30#30
    phantom_dx: float = 0.5e-6
    phantom_fov = phantom_dx * phantom_Nx
    up_samp_fac: int = 1
    # Detector
    det_Nx: int = 16#64  # 32 -- TODO: should have det_N < phantom_N, but need to account for this in phantom init during recon!    
    det_Ny: int = 30#30  # 10
    det_fwhm: float = 1e-6
...
data = np.load(r'/home/aeferretti/rotations/fall/xpc-autodiff-recon/ferretti/tuned_recons/noisy_projection_data_thin_phantom.npy') 
...
w_tv_beta = 966 #000#10
w_tv_delta = 1166 #1614.2858 #10#5
...
in_rand_array = np.zeros((16,30,16,2))
#in_rand_array[:,:,:,0] = np.random.rand(16,30,16)*10**-8 #Starting intial guess set to correct order of magnitude
#in_rand_array[:,:,:,1] = np.random.rand(16,30,16)*10**-6 #Starting intial guess set to correct order of magnitude
in_rand_array[:,:,:,0] = np.ones([16,30,16])*10**-8 #Starting intial guess set to correct order of magnitude
in_rand_array[:,:,:,1] = np.ones([16,30,16])*10**-6 #Starting intial guess set to correct order of magnitude
...
np.save('proj_approx_image_thin',params['params']['volume'])
